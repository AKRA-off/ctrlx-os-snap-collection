...
# Import for storing detection results
try:
    import instance_segmentation
    _can_store_results = True
except ImportError:
    _can_store_results = False
...
def draw_detections_no_nms(detections, img, config_data, labels, arch, tracker=None):
    ...
    # Store detection results for HAILO inference retrieval
    if _can_store_results:
        try:
            # Flatten boxes for storage
            flattened_boxes = np.ravel(original_boxes) if len(original_boxes) > 0 else np.array([])
            flattened_scores = np.ravel(scores) if len(scores) > 0 else np.array([])
            flattened_classes = np.ravel(classes) if len(classes) > 0 else np.array([])
            instance_segmentation.store_detection_results(flattened_boxes, flattened_scores, flattened_classes)
        except Exception as e:
            pass  # Silently ignore if storage fails
