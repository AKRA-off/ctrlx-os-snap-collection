...
int64_t requestLogTransfer(kord::KordCore &, kord::ControlInterface &);

//! Will monitor the status of the request to tranfer logs until time out, failure, or success
bool monitorLogTransfer(kord::KordCore &, kord::ReceiverInterface &, utils::LaunchParameters &, int64_t);
...
int main(int argc, char *argv[])
{
    ...
    request_id = requestLogTransfer(*kord, ctl_iface);
    if (request_id <= 0) {
        KORD_LOG_ERROR("Failed to sent log request");
        return EXIT_FAILURE;
    }

    // Waiting for the execution result
    if (!monitorLogTransfer(*kord, rcv_iface, lp, request_id)) {
...
bool monitorLogTransfer(kord::KordCore &a_kord,
