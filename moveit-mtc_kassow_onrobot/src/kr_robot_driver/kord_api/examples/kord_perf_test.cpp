...
int main(int argc, char *argv[])
{
    ...
    if (rcv_iface.systemAlarmState()) {
        KORD_LOG_ERROR("System alarm state detected, exiting");
        return EXIT_FAILURE;
    }
    ...
    std::ofstream file("api_heartbeat_timestamps.csv");
    if (!file.is_open()) {
        KORD_LOG_ERROR("Failed to open file");
        return EXIT_FAILURE;
    }
    file << "heartbeat_tx_ts,heartbeat_rx_ts,latest_cmd_token\n";
    ...
    while (g_run) {
        ...
        int64_t heartbeat_tx_ts = kord->getTxStamp();
        file << heartbeat_tx_ts << "," << std::chrono::system_clock::now().time_since_epoch().count() << ","
             << rcv_iface.getLatestCommandToken() << "\n";
